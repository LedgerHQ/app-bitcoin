...
static bool derive_compressed_public_key(unsigned char *serialized_path,
...
static bool get_address_from_compressed_public_key(
  ...
  if (cashAddr) {
    ...
    public_key_hash160(compressed_pub_key, // IN
                       33,                 // INLEN
                       tmp);
    if (!cashaddr_encode(tmp, 20, (uint8_t *)address, max_address_length,
                         CASHADDR_P2PKH))
      return false;
    ...
    address_length =
        public_key_to_encoded_base58(compressed_pub_key,     // IN
                                     33,                     // INLEN
                                     (uint8_t *)address,     // OUT
    ...
    if (!nativeSegwit) {
      address_length = public_key_to_encoded_base58(tmp,                // IN
                                                    22,                 // INLEN
                                                    (uint8_t *)address, // OUT
                                                    150, // MAXOUTLEN
...
void swap_handle_check_address(check_address_parameters_t *params) {
  ...
  if (!derive_compressed_public_key(
          params->address_parameters + 1, params->address_parameters_length - 1,
          compressed_public_key, sizeof(compressed_public_key))) {
