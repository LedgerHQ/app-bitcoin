...
static unsigned short sign_message_internal(buffer_t* buffer, uint8_t p1, uint8_t p2) {
    ...
    if (p1 == P1_PREPARE) {
        if ((p2 == P2_FIRST) || (p2 == P2_LEGACY)) {
            ...
            cx_sha256_init_no_throw(&context_D.transactionHashFull.sha256);
            cx_sha256_init_no_throw(&context_D.transactionHashAuthorization);
        ...
        if (message_check_bit_id(context_D.transactionSummary.keyPath) != BITID_NONE) {
