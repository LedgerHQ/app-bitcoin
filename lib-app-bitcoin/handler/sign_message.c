...
unsigned short message_compute_hash(void) {
  ...
  memset(&context.transactionSummary, 0, sizeof(transaction_summary_t));
  return sw;
...
static unsigned short sign_message_internal(buffer_t *buffer, uint8_t p1,
                                            uint8_t p2) {
  unsigned short sw = SW_OK;
  ...
  if (p1 == P1_PREPARE) {
    if ((p2 == P2_FIRST) || (p2 == P2_LEGACY)) {
      ...
      if (p2 == P2_LEGACY) {
        context.transactionSummary.messageLength = buffer->ptr[offset];
        offset++;
      } else {
        context.transactionSummary.messageLength =
            (buffer->ptr[offset] << 8) | (buffer->ptr[offset + 1]);
        offset += 2;
      }
      ...
      // See
      // https://github.com/HorizenOfficial/zen/blob/v5.0.0/src/main.cpp#L122
      const char *magicHeader =
          (COIN_KIND != COIN_KIND_HORIZEN) ? COIN_COINID : "Zcash";

      cx_sha256_init_no_throw(&context.transactionHashFull.sha256);
      cx_sha256_init_no_throw(&context.transactionHashAuthorization);

      chunkLength = strlen(magicHeader) + sizeof(SIGNMAGIC);
      if (cx_hash_no_throw(&context.transactionHashFull.sha256.header, 0,
                           &chunkLength, 1, NULL, 0)) {
        goto discard;
      }
      if (cx_hash_no_throw(&context.transactionHashFull.sha256.header, 0,
                           (uint8_t *)magicHeader, strlen(magicHeader), NULL,
                           0)) {
        sw = SW_TECHNICAL_PROBLEM_2;
        goto discard;
      }
      if (cx_hash_no_throw(&context.transactionHashFull.sha256.header, 0,
                           (unsigned char *)SIGNMAGIC, sizeof(SIGNMAGIC), NULL,
                           0)) {
        sw = SW_TECHNICAL_PROBLEM_2;
        goto discard;
      }
      if (context.transactionSummary.messageLength < 0xfd) {
        messageLength[0] = context.transactionSummary.messageLength;
        messageLengthSize = 1;
      } else {
        messageLength[0] = 0xfd;
        messageLength[1] = (context.transactionSummary.messageLength & 0xff);
        messageLength[2] =
            ((context.transactionSummary.messageLength >> 8) & 0xff);
        messageLengthSize = 3;
      }
      if (cx_hash_no_throw(&context.transactionHashFull.sha256.header, 0,
    ...
    if (message_check_bit_id(context.transactionSummary.keyPath) !=
        BITID_NONE) {
