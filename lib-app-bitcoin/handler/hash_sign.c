...
WEAK unsigned short handler_hash_sign(buffer_t* buffer, uint8_t p1, uint8_t p2) {
...
#define HASH_LENGTH 1 + 1 + 4 + 1
    ...
    memmove(context.transactionSummary.keyPath,
            buffer->ptr,
...
int user_action_signtx(unsigned char confirming, unsigned char direct) {
    ...
    if (confirming) {
        ...
        if (context.usingOverwinter) {
            LEDGER_ASSERT(cx_hash_no_throw(&context.transactionHashFull.blake2b.header, CX_LAST, hash, 0, hash, 32) == CX_OK, "Hash Failed");
        }
        else {
            LEDGER_ASSERT(cx_hash_no_throw(&context.transactionHashFull.sha256.header, CX_LAST,
        ...
        G_io_apdu_buffer[context.outLength++] = context.transactionSummary.sighashType;
