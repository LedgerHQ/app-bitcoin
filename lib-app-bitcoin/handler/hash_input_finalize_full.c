...
static unsigned short hash_input_finalize_full_internal(transaction_summary_t *transactionSummary, buffer_t* buffer, uint8_t p1, uint8_t p2, bool *async) {
    ...
    if (!context_D.segwitParsedOnce) {
        ...
        if (context_D.usingOverwinter) {
            if (cx_hash_no_throw(&context_D.transactionHashFull.blake2b.header, 0, buffer->ptr + hashOffset, buffer->size - hashOffset, NULL, 0)) {
                sw = SW_TECHNICAL_PROBLEM_2;
                goto discardTransaction;
            }
        }
    ...
    if (context_D.transactionContext.firstSigned) {
        if ((context_D.currentOutputOffset + buffer->size) >
        ...
        context_D.currentOutputOffset += buffer->size;
...
unsigned short handler_hash_input_finalize_full(buffer_t* buffer, uint8_t p1, uint8_t p2) {
    ...
    unsigned short sw = hash_input_finalize_full_internal(
        &context_D.transactionSummary, buffer, p1, p2, &is_async);
    ...
    return io_send_response_pointer(G_io_apdu_buffer, context_D.outLength, sw);
}
