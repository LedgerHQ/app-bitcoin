...
static int check_output_displayable(bool* displayable) {
    *displayable = true;
    ...
    isOpReturn =
        output_script_is_op_return(context.currentOutput + 8);
    isP2sh = output_script_is_p2sh(context.currentOutput + 8);
    isNativeSegwit = output_script_is_native_witness(
        context.currentOutput + 8);
#ifndef __clang_analyzer__
    unsigned char isOpCreate =
        output_script_is_op_create(context.currentOutput + 8,
          sizeof(context.currentOutput) - 8);
    unsigned char isOpCall =
        output_script_is_op_call(context.currentOutput + 8,
          sizeof(context.currentOutput) - 8);
    if (((COIN_KIND == COIN_KIND_HYDRA) &&
         !output_script_is_regular(context.currentOutput + 8) &&
    ...
    if (context.tmpCtx.output.changeInitialized && !isOpReturn) {
        ...
        if (!isP2sh &&
            ...
            memmove(changeSegwit + 2,
                       context.tmpCtx.output.changeAddress, 20);
            public_key_hash160(changeSegwit, 22, changeSegwit);
            if (memcmp(context.currentOutput + 8 + addressOffset,
                          changeSegwit, 20) == 0) {
    ...
    *processed = 0;
    ...
    case OUTPUT_PARSING_NUMBER_OUTPUTS: {
        ...
        if (context.currentOutput[0] == 0xFD) {
            ...
            context.totalOutputs = context.remainingOutputs =
                (context.currentOutput[2] << 8) |
                context.currentOutput[1];
            discardSize = 3;
            context.outputParsingState = OUTPUT_PARSING_OUTPUT;
            *processed = 1;
            break;
    ...
    case OUTPUT_PARSING_OUTPUT: {
        ...
        if (context.currentOutputOffset <
    ...
    if (discardSize != 0) {
        memmove(context.currentOutput,
                   context.currentOutput + discardSize,
...
static unsigned short hash_input_finalize_full_internal(transaction_summary_t *transactionSummary, buffer_t* buffer, uint8_t p1, uint8_t p2, bool *async) {
    ...
    if (context.usingSegwit &&
        (context.tmpCtx.output.multipleOutput == 0)) {
    ...
    if (p1 == FINALIZE_P1_CHANGEINFO) {
        ...
        if (get_pubkey_hash160(transactionSummary->keyPath, sizeof(transactionSummary->keyPath), context.tmpCtx.output.changeAddress)) {
    ...
    if (!context.segwitParsedOnce) {
        ...
        if (context.usingOverwinter) {
            if (cx_hash_no_throw(&context.transactionHashFull.blake2b.header, 0, buffer->ptr + hashOffset, buffer->size - hashOffset, NULL, 0)) {
                sw = SW_TECHNICAL_PROBLEM_2;
                goto discardTransaction;
            }
        }
        else {
            PRINTF("--- ADD TO HASH FULL:\n%.*H\n", buffer->size - hashOffset, buffer->ptr + hashOffset);
            if (cx_hash_no_throw(&context.transactionHashFull.sha256.header, 0,
                        buffer->ptr + hashOffset,
                        buffer->size - hashOffset, NULL, 0)) {
                sw = SW_TECHNICAL_PROBLEM_2;
                goto discardTransaction;
            }
        }
    ...
    if (!context.usingSegwit) {
        ...
        if (cx_hash_no_throw(&context.transactionHashAuthorization.header,
                    CX_LAST, buffer->ptr,
                    buffer->size, authorizationHash, 32)) {
            sw = SW_TECHNICAL_PROBLEM_2;
            goto discardTransaction;
        }
    }
    ...
    if (context.usingSegwit) {
        if (!context.segwitParsedOnce) {
            if (context.usingOverwinter) {
                if (cx_hash_no_throw(&context.transactionHashFull.blake2b.header, CX_LAST, context.segwit.cache.hashedOutputs, 0, context.segwit.cache.hashedOutputs, 32)) {
                    sw = SW_TECHNICAL_PROBLEM_2;
                    goto discardTransaction;
                }
            }
                ...
                if (cx_hash_no_throw(&context.transactionHashFull.sha256.header,
                            CX_LAST,
                            context.segwit.cache.hashedOutputs, 0,
...
discardTransaction:
    ...
    memmove(G_io_apdu_buffer, context.currentOutput,
            context.currentOutputOffset);
