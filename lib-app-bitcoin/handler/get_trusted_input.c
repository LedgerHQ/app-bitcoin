...
WEAK unsigned short handler_get_trusted_input(buffer_t *buffer, uint8_t p1,
  ...
  if (p1 == GET_TRUSTED_INPUT_P1_FIRST) {
    // Initialize
    context.transactionTargetInput = read_u32_be(buffer->ptr, 0);
  ...
  if (context.transactionContext.transactionState == TRANSACTION_PARSED) {
    ...
    cx_hash_sha256(G_io_apdu_buffer + TRUSTED_INPUT_SIZE, 32,
                   G_io_apdu_buffer + 4, 32);

    write_u32_le(G_io_apdu_buffer, 4 + 32, context.transactionTargetInput);
    ...
    cx_hmac_sha256((uint8_t *)g_nvram_data.bkp.trustedinput_key,
                   ...
                   TRUSTED_INPUT_SIZE, G_io_apdu_buffer + TRUSTED_INPUT_SIZE,
                   32);
