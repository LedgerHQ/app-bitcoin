...
static int get_public_key_chain_code(const unsigned char* keyPath, size_t keyPath_len, unsigned char* publicKey, unsigned char* chainCode) {
...
WEAK unsigned short handler_get_wallet_public_key(buffer_t* buffer, uint8_t p1, uint8_t p2) {
    ...
    if (p2 != P2_NATIVE_SEGWIT && p2 != P2_LEGACY && p2 != P2_SEGWIT && p2 != P2_CASHADDR) {
        PRINTF("Wrong P2 value\n");
        return io_send_sw(SW_INCORRECT_P1_P2);
    }
    ...
    keyLength = get_public_key_chain_code(buffer->ptr, MAX_BIP32_PATH_LENGTH, G_io_apdu_buffer + 1, chainCode);
