...
WEAK unsigned short handler_hash_input_start(buffer_t* buffer, uint8_t p1, uint8_t p2) {
    ...
    if ((p2 == P2_NEW) ||
        ...
        if (p1 == P1_FIRST) {
            ...
            if (COIN_KIND == COIN_KIND_BITCOIN_CASH) {
                ...
                context_D.usingCashAddr = usingCashAddr;
            }
            else {
                context_D.usingCashAddr = 0;
            }
        ...
        && !IS_INPUT_TRUSTED())
    {
        ...
        context_D.transactionBufferPointer = (uint8_t*) buffer->ptr;
        context_D.transactionDataRemaining = buffer->size;
