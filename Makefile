...
APP_LOAD_PARAMS += --path $(APP_PATH)
else ifeq ($(COIN),bitcore)
# Bitcore
DEFINES   += COIN_P2PKH_VERSION=3 COIN_P2SH_VERSION=125 COIN_FAMILY=1 COIN_COINID=\"Bitcore\" COIN_COINID_HEADER=\"BITCORE\" COIN_COLOR_HDR=0xC831A7 COIN_COLOR_DB=0xce774ce COIN_COINID_NAME=\"Bitcore\" COIN_COINID_SHORT=\"BTX\" COIN_KIND=COIN_KIND_BITCORE COIN_FORKID=0 COIN_FLAGS=FLAG_SEGWIT_CHANGE_SUPPORT
APPNAME ="BitCore"
APP_LOAD_PARAMS += --path $(APP_PATH)
...
$(error Unsupported COIN - use bitcoin_testnet, bitcoin, bitcoin_cash, bitcoin_gold, litecoin, dogecoin, dash, zcash, horizen, komodo, stratis, peercoin, pivx, viacoin, vertcoin, stealth, digibyte, qtum, bitcoin_private, zcoin, gamecredits, zclassic, xsn, nix, lbry, resistance, bitcore)
endif
endif
...
listvariants:
	@echo VARIANTS COIN bitcoin_testnet bitcoin bitcoin_cash bitcoin_gold litecoin dogecoin dash zcash horizen komodo stratis peercoin pivx viacoin vertcoin stealth digibyte qtum bitcoin_private zcoin gamecredits zclassic xsn nix lbry bitcore

else

listvariants:
	@echo VARIANTS COIN bitcoin_testnet bitcoin bitcoin_cash bitcoin_gold litecoin dogecoin dash zcash horizen komodo stratis peercoin pivx viacoin vertcoin stealth digibyte qtum bitcoin_private zcoin gamecredits zclassic xsn nix lbry resistance bitcore
