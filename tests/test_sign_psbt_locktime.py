...
def sign(navigator: Navigator, firmware: Firmware, client: RaggerClient, test_name: str,
    ...
    return client.sign_psbt(
        ...
        None,
        navigator,
        # save_screenshot=False: the lock time is not displayed, so there is nothing new to
        # compare, and these tests would otherwise each want their own snapshot directory.
...
def test_locktime_oracle_rejects_the_wrong_locktime(navigator: Navigator, firmware: Firmware,
    ...
    """The check above has teeth: verifying against a different lock time must fail."""
