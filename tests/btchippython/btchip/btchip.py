...
class btchip:
	...
	@classmethod
	def getTrustedInput(self, cmd, transaction, index):
		result = {}
		# Header
		...
		apdu.extend(params)
		cmd.transport.exchange_raw(bytearray(apdu))
		# Each input
	...
	@classmethod
	def startUntrustedTransaction(self, cmd, newTransaction, inputIndex, outputList, redeemScript, version=0x01, cashAddr=False, continueSegwit=False):
	...
	@classmethod
	def finalizeInput(self, cmd, outputAddress, amount, fees, changePath, rawTx=None):
	...
	@classmethod
	def untrustedHashSign(self, cmd, path, pin="", lockTime=0, sighashType=0x01):
		if isinstance(pin, str):
			pin = pin.encode('utf-8')
		donglePath = parse_bip32_path(path)
		...
		apdu.append(len(params))
		apdu.extend(params)
		_,result = cmd.transport.exchange_raw(bytearray(apdu))
	...
	@classmethod
	def enableAlternate2fa(self, cmd, persistent):
		if persistent:
			p1 = 0x02
		else:
			p1 = 0x01
