...
def _wpkh_1in1out(sighash: int) -> PSBT:
    # Derive a 1-input / 1-output P2WPKH tx from wpkh-1to2 by dropping the change
    # output and keeping the single external payment (resized to preserve the small
    # original fee). Used to exercise the SINGLE output-count carve-out.
    psbt = open_psbt_from_file(f"{tests_root}/psbt/singlesig/wpkh-1to2.psbt")
...
def test_sighash_single_1in1out_spent_only(navigator: Navigator, firmware: Firmware, client: RaggerClient, test_name: str):
    # SINGLE (0x03) carve-out: with a single provided output the signature commits it, so the
    # review is SPENT_ONLY (You spend / Fees: Not available / Signing rule) instead of the
    # UNAVAILABLE notice shown when SINGLE leaves >1 output uncommitted. A non-default sighash
    # never makes the fee trustworthy, so it is never a full review.
    toggle_nonstandard_sighash_setting(navigator, firmware)
...
def test_sighash_single_anyonecanpay_1in1out_spent_only(navigator: Navigator, firmware: Firmware, client: RaggerClient, test_name: str):
    # ANYONECANPAY|SINGLE (0x83) with a single output: the output is committed (SINGLE
    # carve-out) but the open input set makes the fee untrustworthy -> SPENT_ONLY. This is
...
def test_sighash_registered_wallet_spent_only(navigator: Navigator, firmware: Firmware, client: RaggerClient, test_name: str):
    # Registered (non-default) wallet policy: the account row shows the registered wallet
    # name instead of a "<script> account #n" default label. ANYONECANPAY|ALL (0x81) ->
    # SPENT_ONLY, so the registered-name row is exercised in a trust-reduced review.
    toggle_nonstandard_sighash_setting(navigator, firmware)
