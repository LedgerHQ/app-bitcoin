        ...
        if (btchip_context_D.outputParsingState ==
            ...
            if (btchip_context_D.usingSegwit &&
                ...
                if (btchip_context_D.NU5Transaction) {
                    ...
                    os_memmove(btchip_context_D.nu5_ctx.prevouts_sig_digest, btchip_context_D.segwit.cache.hashedPrevouts, DIGEST_SIZE);
                    // Save seq hash in s2e
                    os_memmove(btchip_context_D.nu5_ctx.sequence_sig_digest, btchip_context_D.segwit.cache.hashedSequence, DIGEST_SIZE);
                    // Save outputs hash in s2f
                    os_memmove(btchip_context_D.nu5_ctx.outputs_sig_digest, btchip_context_D.segwit.cache.hashedOutputs, 32);
                }
                btchip_context_D.transactionContext.transactionState =
                    BTCHIP_TRANSACTION_NONE;
            } else {
