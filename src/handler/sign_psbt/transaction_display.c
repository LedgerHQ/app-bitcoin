...
bool __attribute__((noinline)) display_transaction(
    ...
    if (st->warnings.non_default_sighash) {
        if (st->sighash_mixed) {
            ...
            mode = decide_tx_display_mode(fee_trustworthy, outputs_committed, amounts_trustworthy);
        }
    }
    ...
    // From when net-spending, To when net-receiving, neutral when unknown (UNAVAILABLE).
    ...
    if (st->n_external_outputs <= MAX_EXT_OUTPUT_SIMPLIFIED_NUMBER) {
        ...
        // In NET_ONLY "You spend/receive" already states the amount; a "0 (self-transfer)"
        // row would contradict a net-receive, so omit it.
