 ...
 * key placeholder in the provided policy. Key indices outside that range are rejected as an error,
 * as they do not correspond to any key info actually supplied by the caller.
 *
 ...
 *   error, including if a placeholder references an index outside of that range.
 */
